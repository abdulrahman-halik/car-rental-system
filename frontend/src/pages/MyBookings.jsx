                        ...
                        <div className="flex-1 w-full grid grid-cols-1 md:grid-cols-3 gap-6">
                            ...
                            <div className="md:col-span-1 space-y-1">
                                ...
                                <div className="flex items-center gap-1.5 text-sm text-gray-500 mt-2">
                                    <img src={assets.location_icon_colored} alt="Location" className="w-4 h-4 opacity-70" />
                            ...
                            <div className="md:col-span-1 flex flex-col justify-center space-y-3">
                                <div>
                                    <p className="text-xs text-gray-400 uppercase tracking-wide font-semibold mb-1">Rental Period</p>
                                    <div className="flex items-center gap-2 text-sm text-gray-700 font-medium">
                                        <img src={assets.calendar_icon_colored} alt="Date" className="w-4 h-4" />
                                        <span>
                                            {booking.pickupDate.split("T")[0]} <span className="text-gray-400 mx-1">→</span> {booking.returnDate.split("T")[0]}
                                        </span>
                                    </div>
                                </div>
                                <div>
                                    <span className={`inline-flex items-center px-2.5 py-1 rounded-full text-xs font-medium capitalize border ${booking.status === "confirmed"
                                        ? "bg-green-50 text-green-700 border-green-200"
                                        : "bg-amber-50 text-amber-700 border-amber-200"
                                        }`}>
                                        <span className={`w-1.5 h-1.5 rounded-full mr-1.5 ${booking.status === "confirmed" ? "bg-green-600" : "bg-amber-600"
                                            }`}></span>
                                        {booking.status}
                                    </span>
                                </div>
