                    ...
                    <div className="group relative">
                        <label htmlFor="image" className="cursor-pointer block relative">
                            ...
                            <div className="absolute inset-0 bg-black/40 rounded-full flex items-center justify-center opacity-0 group-hover:opacity-100 transition-opacity backdrop-blur-sm">
                                <img src={assets.edit_icon} alt="Edit" className="w-8 h-8 invert opacity-80" />
                            </div>
                            <div className={`absolute bottom-1 right-1 w-6 h-6 rounded-full border-4 border-white ${isOnline ? 'bg-green-500' : 'bg-gray-300'}`}></div>
                        </label>
                    </div>
                    ...
                    {ownerMenuLinks.map((link, index) => {
                        ...
                        return (
                            ...
                            >
                                ...
                                <span className={`font-semibold tracking-wide ${isActive ? '' : 'group-hover:tracking-wider transition-all'}`}>{link.name}</span>
